...
let defineColorByPosition = (position: number, matchweek: number) => {
  if (position === 1 && matchweek > 37){
    return 'gold'
  }
  if (position <= 4) {
    return '#b3ffcc'
        ...
        <Table className={classes.table} size="small">
          ...
          <TableBody>
            {dataForRender.map((row, index) => (
              <Tooltip title={index < 4 ? 'Champions League spots' : (index > 16 ? 'Relegation' : '')} placement="top-start"><TableRow key={index + 1} style={{ backgroundColor: defineColorByPosition(index + 1, props.matchweek), height: '30px' }}>
